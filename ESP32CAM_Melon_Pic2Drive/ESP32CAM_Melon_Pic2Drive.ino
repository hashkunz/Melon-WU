...
String myDeploymentID = "xxxxxxxxxxxxxxxxxxxxxxxx";
...
bool set = false;
bool ready = false;
String weekDays[7] = {"วันอาทิตย์", "วันจันทร์", "วันอังคาร", "วันพุธ", "วันพฤหัสบดี", "วันศุกร์", "วันเสาร์"};
...
void setupWiFi() {
    WiFiManager wm;
    bool res;
    res = wm.autoConnect("Melon MAI CAM", "87654321");
    if (!res) {
...
void setup() {
  ...
  LINE.notify("ระบบจะทำการแจ้งเตือนเมื่อถึงเวลา 07.00 / 10.00 / 13.00 / 17.00 เท่านั่น");
...
void sendLineNotify() {
  String messageT = weekDay + " ถ่ายรอบที่ " + count + " สำเร็จ!";
  String messageF = weekDay + " ถ่ายรอบที่ " + count + " ไม่สำเร็จ";
  count = count + 1;
  if (set) {
    LINE.notify(messageT);
    LINE.notify("สามารถตรวจสอบรูปภาพจากลิงค์นี้ : xxxxxxxxxxxxxxxxxxxxxxxx");
    LINE.notifySticker("ส๊วดยอดดดดดดด", 1070, 17844); // ส่ง Line Sticker ด้วย PackageID 1 , StickerID 2  พร้อมข้อความ
  } else {
    LINE.notify(messageF);
    LINE.notifySticker("ถามจริ๊งงงงงงงงง", 1070, 17873); // ส่ง Line Sticker ด้วย PackageID 1 , StickerID 2  พร้อมข้อความ
    LINE.notify("โปรดไปตรวจสอบการเชื่อมต่อของเซ็นเซอร์...... @HashKunz");
  }
}

// ready คือ ตัวแปรที่จะทำให้เกิดการถ่ายรูป
// set คือ ตัวแปรที่จะทำการส่ง respone ไปที่ไลน์ว่าถ่ายสำเร็จไหม
...
void loop() {
    ...
    if ((hour == 7 || hour == 10 || hour == 13  || hour == 17) && minute == 0) {
      ready = true;
      Serial.println("Camera Ready : True");
    } else {
      count = 0;
      ready = false;
      Serial.println("Camera Ready : False");
    }
    ...
    if (ready) {
      ...
      Serial.println("Now is not time!!");
    }
    delay(20000);
}
